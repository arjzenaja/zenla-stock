...
import { Camera, X, DollarSign, Package, Banknote, Loader2 } from 'lucide-react'
...
export const AddProductForm: React.FC = () => {
  ...
  const mutation = useMutation({
    mutationFn: async (values: AddProductFormValues) => {
      // Auto-generate SKU if not provided (though not in schema yet, I'll let DB handle or generate here)
      ...
      const res = await fetch('/api/products', {
        ...
        body: JSON.stringify({
          ...values,
          sku,
          categoryId: values.category, // Map 'category' field from form to 'categoryId' in DB
          currentStock: values.initialStock,
          status: values.initialStock > 10 ? 'safe' : values.initialStock > 0 ? 'low' : 'out_of_stock',
          stockHealth: values.initialStock > 10 ? 'stable' : 'low',
          image: imagePreview,
        }),
      })
        ...
        <div className="bg-white rounded-2xl shadow-card p-6 md:p-8 border border-slate-50">
          ...
          <div className="space-y-6">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-xs font-bold tracking-widest text-slate-400 uppercase">Stok Awal</FormLabel>
                  <FormControl>
                    <QuantityStepper 
                      value={field.value} 
